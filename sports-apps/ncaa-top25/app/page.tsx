...
export default function Home(){
 ...
 useEffect(()=>{
  ...
  const loadDetails=async()=>{
   try{
    const response=await fetch(`/api/college-football?gameId=${encodeURIComponent(selectedGameId)}`,{cache:'no-store'});
 ...
 const toggleGame=async(game:ScheduleGame,selectedTeam:typeof teams[number],force=false)=>{
  if(!force&&expandedGameId===game.id){setExpandedGameId(null);return}
  setExpandedGameId(game.id);if(game.state!=='post'){setLegacyLoading(null);return}if(legacyDetails[game.id]&&!force)return;setLegacyLoading(game.id);
  try{
   ...
   let scoringSource='ESPN',scoringSourceUrl='',scoringUpdatedAt='';
   ...
   if(normalizedTeams.some((team,index)=>scoringTotal(index)!==(Number(team.score)||0))){
    try{
  ...
  }catch{}finally{setLegacyLoading(null)}
 };
   ...
   <section className="content-grid">
    <article className="rankings panel anchor" id="rankings">
     ...
     {filtered.map((t,i)=><div className="team-entry" key={t.abbr}>{(i===0||conferenceOf(filtered[i-1])!==conferenceOf(t))&&<div className="conference-heading">{conferenceOf(t)}</div>}
      <button onClick={()=>toggleSchedule(t)} aria-expanded={expanded===t.abbr} aria-controls={`schedule-${t.abbr}`} className={selected===t.abbr?'team-row selected':'team-row'}><b>{t.rank}</b><span className="team-name"><i className="team-logo" style={{background:t.color}}><span>{t.abbr}</span><img src={`https://a.espncdn.com/i/teamlogos/ncaa/500/${t.espnId}.png`} alt={`${t.name} logo`} onError={event=>{event.currentTarget.style.display='none'}} /></i><span><strong>{t.name}</strong><small>{t.abbr}</small><small className="weekly-score">{weeklyScores[t.espnId]?`THIS WEEK · ${weeklyScores[t.espnId]}`:'NO GAME THIS WEEK'}</small></span></span><span>{currentRecords[t.espnId]??t.record}</span><strong>{t.points.toLocaleString()}</strong><span className="schedule-cta">{expanded===t.abbr?'Close':'View'} <i>{expanded===t.abbr?'↑':'↓'}</i></span></button>
     {expanded===t.abbr&&<div className="schedule-panel" id={`schedule-${t.abbr}`}><div className="schedule-head"><span><b>{t.name}</b> · 2026 SCHEDULE</span><small>{scheduleLoading===t.abbr?'LOADING…':`${schedules[t.abbr]?.length??0} GAMES`}</small></div>{scheduleLoading===t.abbr?<p className="schedule-message">Loading the latest schedule…</p>:scheduleError===t.abbr?<p className="schedule-message">Schedule could not be loaded. Close and try again.</p>:(schedules[t.abbr]??[]).map(game=><div className="schedule-game-wrap" key={game.id}><button className="schedule-game" onClick={()=>toggleGame(game,t)} aria-expanded={expandedGameId===game.id}><time dateTime={game.date}>{new Date(game.date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</time><span>{game.neutral?'vs':game.venue} <b>{game.opponent}</b></span><small>{mountainTime(new Date(game.date))} MT · {expandedGameId===game.id?'Close':game.state==='post'?'Stats':'Details'} {expandedGameId===game.id?'↑':'↓'}</small></button>{expandedGameId===game.id&&<div className="legacy-game-panel">{game.state!=='post'?<p className="schedule-message"><b>{game.status}</b> · {mountainTime(new Date(game.date))} MT{game.network?` · ${game.network}`:''}<br/>Game statistics will appear after the game is completed.</p>:legacyLoading===game.id?<p className="schedule-message">Loading archived game statistics…</p>:legacyDetails[game.id]?<><p className="archive-label">GAME ARCHIVE · {legacyDetails[game.id].headline}</p><div className="team-archive-grid">{legacyDetails[game.id].teams.map((team,index)=><section className="team-archive" key={team.id}><header><span><small>{team.id===t.espnId?'SELECTED TEAM':'OPPONENT'}</small><b>{team.name}</b></span><strong>{team.score}</strong></header><div className="team-stat-list">{legacyDetails[game.id].stats.map(stat=><div key={stat.name}><span>{stat.name}</span><b>{index===0?stat.away:stat.home}</b></div>)}{legacyDetails[game.id].touchdowns&&<div><span>Touchdowns</span><b>{legacyDetails[game.id].touchdowns[index]}</b></div>}{legacyDetails[game.id].fieldGoals&&<div><span>Field goals</span><b>{legacyDetails[game.id].fieldGoals[index]}</b></div>}{legacyDetails[game.id].extraPoints&&<div><span>Extra points</span><b>{legacyDetails[game.id].extraPoints[index]}</b></div>}{legacyDetails[game.id].twoPointConversions&&<div><span>2-point conversions</span><b>{legacyDetails[game.id].twoPointConversions[index]}</b></div>}{legacyDetails[game.id].safeties&&<div><span>Safeties</span><b>{legacyDetails[game.id].safeties[index]}</b></div>}<div><span>Scoring check</span><b>{legacyDetails[game.id].scoringVerified?.[index]?'✓ Matches':'⚠ Review'}</b></div></div><div className="team-leaders"><p>PLAYER LEADERS</p>{legacyDetails[game.id].leaders?.filter(leader=>leader.teamId===team.id).length?legacyDetails[game.id].leaders?.filter(leader=>leader.teamId===team.id).map(leader=><div key={`${leader.category}-${leader.name}`}><span><small>{leader.category}</small>{leader.name}</span><b>{leader.value}</b></div>):<small className="leaders-pending">Player leaders have not been posted by the provider.</small>}</div></section>)}</div><p className="archive-label">SCORING SOURCE · {legacyDetails[game.id].scoringSource??'ESPN'}</p>{legacyDetails[game.id].lastPlay&&<p className="last-play"><b>LAST PLAY:</b> {legacyDetails[game.id].lastPlay}</p>}</>:<p className="schedule-message">No archived statistics are available for this game yet.</p>}</div>}</div>)}</div>}
     </div>)}
    </article>
